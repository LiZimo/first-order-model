...
def single_pass(source, initial_image, target_image, generator, kp_detector, relative=True, adapt_movement_scale=True, cpu=False):
	with torch.no_grad():
		#predictions = []
		#source = torch.tensor(source_image[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
...
def stream_to_webcam(source_image, reader, generator, kp_detector, face_alignment_instance, relative=True, adapt_movement_scale=True, cpu=False, bbox_avg_frames = 10, check_ref_frame_rate = 3):
	bbox_queue = []
	with torch.no_grad():
		#predictions = []
		source = torch.tensor(source_image[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
...
def stream_to_webcam_multisource(source_dir, reader, generator, kp_detector, face_alignment_instance, relative=True, adapt_movement_scale=True, cpu=False, bbox_avg_frames = 10, check_ref_frame_rate = 3, cycle_time = 30):
	bbox_queue = []
	...
	with torch.no_grad():
		...
		for i, frame in tqdm(enumerate(reader)):
			...
			if i % cycle_time == 0:
				source_files = glob(source_dir + '/**/*.*', recursive=True)
				source_image = imageio.imread(source_files[source_files_ind])
			...
			if i % check_ref_frame_rate == 0:
				...
				indices = [3,4,12,13] + list(range(48,60))
...
if __name__ == "__main__":
	...
	parser.add_argument("--cycle_time", default=10, help="time in seconds of changing source image, if source_dir is given")
	...
	if not opt.webcam:
		...
		if opt.source_dir is None:
			...
			stream_to_webcam_multisource(opt.source_dir, reader, generator, kp_detector, fa, relative=opt.relative, adapt_movement_scale=opt.adapt_scale, cpu=opt.cpu, cycle_time= int (opt.cycle_time * 3))